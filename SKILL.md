...
6. **凭证先验后问**：**命中资产且其路由/文档声明需要登录**时——取资产路径 → 判「**凭证已在位？**（本地缓存 ｜ 用户提供）」：在位 → 按其〈登录〉说明**验证**后使用 ✓；不在位 → 按其说明走**登录流程**（有现成登录器→调登录器；否则按其说明引导用户提供）；**未验证不得断言「缺凭证」、不得向用户索要** ✗；**禁自写登录流程** ✗；失败按 `control.md`〈卡壳处置〉（≤3 重走 → 问用户）；凭证状态逐资产独立 ✗。
