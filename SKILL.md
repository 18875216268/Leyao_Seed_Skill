...
6. **凭证先验后问**：**命中资产且其路由/文档声明需要登录**时——取资产路径（**唯二来源**：`asset.py resolve <节点id>` ｜ `ROUTES.md` 的 `→ 挂载`；**禁其它形式** ✗）→ 判「**凭证已在位？**（本地缓存 ｜ 用户提供）」：在位 → 按其〈登录〉说明**验证**后使用 ✓；不在位 → 按其说明走**登录流程**（有现成登录器→**必须调登录器并等登录成功**（其自动落库）；否则按其说明引导用户提供）；**未验证不得断言「缺凭证」、不得向用户索要** ✗；**禁自写登录流程** ✗；**凭证取用一律经登录器工具**（**禁止 AI 自行获取** ✗）；**失效处置：主取数壳自动调登录器 ✓；其余情形 agent 主动调登录器（或引导用户提供新凭证）** ✓；**用户已明示登录方式 / 凭证来源 → 按其走** ✓；失败按 `control.md`〈卡壳处置〉〈问题未解决流程〉（≤3 重走 → 问用户）；凭证状态逐资产独立 ✗。
