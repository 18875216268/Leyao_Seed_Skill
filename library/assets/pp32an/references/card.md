...
刷新业务速查卡并做能力库蒸馏与就绪复核：① 按本规范跑 `fetch` → 蒸馏（业务条目 + `@` 基础卡条目一并蒸，只处理 diff）→ `check --nodes library/routes.json` → `render`；② 按 `<技能包>/library/assets/bvix9o/维护.md` 提炼【经验/场景/技能】候选（四门槛：≥2 次复现 · 可执行 · 有坑或风险 · 未被覆盖），去重后**只生成提案**（≤3 条，附证据指针），**不直接改资产**；③ 按 `version/VERSION.md` 复核就绪两件（宿主常驻在位 · 版本检测；在位则静默）；④ 全程失败如实标注、不阻断。
