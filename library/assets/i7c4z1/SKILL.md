...
| --- | --- | --- | --- | --- |
| **A 本包登录（默认 ✓）** | 常规取数 | `python library/assets/i7c4z1/scripts/login_bi.py --reuse` | ``%LOCALAPPDATA%\bi-operations-query\accounts\<loginId>.json`` | **仅本 skill 与其优化板块子 skill** ✓ |
| **B 集团方式（备用）** | 用户**明确要求**集团取数 / 自带 PAT | `vendor/bi-pat`（PAT SQL 通道；`pat_call.py` **自动检查 guancli 登录态**；PAT（`gdpat_`）由**用户领取传入**） | 由集团方管理 | **仅集团文档所述的取数** ✓；**不得用于本包** ✗ |

- 做法：先 `--status`（**只验证、绝不弹窗**）→ 有效即**复用** ✓；失效才 `--reuse`/扫码（扫码成功**自动落库** ✓）
- 取用：优先用**资产出口命令**（BI 见 §1.1（`--status` 默认不含令牌明文，`--show-token` 才输出））；直读仓库为备选；**令牌明文不得回显** ✗
- 无界面 / 自动化：`--no-ui`（不可弹窗）→ 由用户提供凭证，或走 B ✗（B 启用条件见 §1.1 / `vendor/SUBSKILL_ROUTING.md` §1）
- B 的方式：**按其包内文档实时读取、原样执行** ✗（零假设、零转述、不得自研 ✗）
- **红线**：禁自研登录 / 自取二维码 / 自拼鉴权 ✗ · **两套凭证不得交叉使用** ✗ · 参数 / 权限 / 限流 / 网络类报错**不触发登录** ✗
...
2. **登录/凭证**：**登录做法、双鉴权与消费范围一律按本文件顶部〈登录（唯一入口）〉执行**（任何子包都不发起登录 ✗）。**优化 skill 不实现登录，其凭证统一由本 skill 自有登录组件提供，或由用户直接给定**（详见 `vendor/SUBSKILL_ROUTING.md` §6 第 6 条）。
...
- **备用路径**：自有组件不可用（如无界面且无法扫码）时，可按集团子包的鉴权流程走（按 `vendor/bi-pat/SKILL.md` 的鉴权说明执行），操作细节完全遵照集团包文档——备用不等于转述，本框架不做任何假设。
- 用法：
...
- PyQt5 为可选依赖：只有弹扫码窗才需要；**通道工具 `bi_call.py` 在本地凭证缺失 / 过期时默认允许弹窗**——无界面 / 自动化环境请显式 `--no-ui` ✓。
...
1. **登录**：`python scripts/login_bi.py`（`--reuse`：有效即复用，失效才弹扫码）或 `--status`（只验证不弹窗）。产出 user（登录人身份）。
...
- **401 / token 失效**：先 `--status`（只验证、绝不弹窗）确认：有效即复用；失效才 `python scripts/login_bi.py` 扫码重登，不要改代码；弹窗失败可点容器重试。
