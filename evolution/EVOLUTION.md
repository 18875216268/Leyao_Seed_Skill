...
| 证 | `grow.py review` | 观察期结算：促进/降级/淘汰（回滚属「行」环的失败路径） | **结构自检 `run_checks.py` 自动一票否决**（触发评测 / 更新沙箱为**发布前人工回归**，见 `evolution/tests/README.md`）；分数降自动回滚。**不可写面度量**（MetaRSI 定律四：闭环内部"放宽标准"与"真变强"同分且不可自察）→ 结构自检只是**可写面**；真实增益须由**不可写面**给出：用户反馈 / 独立代理实测 / 真实交付成绩（三者任一为负 → 不判"进步" ✗）|
