...
7. 官方规范校验：`python -m skills_ref.cli validate .`（需 `pip install skills-ref`）→ 输出 `Valid skill`
8. 目录结构与 `SKILL.md` 层级导航一致；无 `__pycache__` 残留
9. 运行时数据与评测记录只存用户区（与 skill 同级 `.leyao-data/`，可用 `LEYAO_SEED_HOME` 覆盖），不进交付（其中审计日志与墓碑记录永不清理，随框架整体迁移）
10. 发版：`manifest.json` 与 `SKILL.md` 版本同步（`version_sync` 守住）；宿主常驻与更新流程见 `version/VERSION.md`
