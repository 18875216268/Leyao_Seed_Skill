...
判据链在**真实描述**上的分支回归 + 描述分级静态校验，**以脚本实际输出为准（当前 29 项）**：**A 静态（9）**（描述非空 / 无旧措辞残留 / 结构化节点【回退】【别名】非空 / 别名≠标题 / 业务节点互斥指引 / 判据含 1c / shapes 含路由决策记录 / D 区形状 / 自由描述降级标注在场）·
...
| `default_asset` | 默认资产**呈现面 + 判据面**：`ROUTES.md` 有 ★ 行且含节点 id · 判据 0.5 在场（未注册时合法跳过；**有默认层时：★ 默认层行成员/标签与 defaults.layers 一致**、**能力库索引 ↔ 场景文件一致**；**注册合法性归 `routes_contract`**、**卡文件健康归资产 `card.py check`**——不重复判） |
