...
| `default_asset` | 默认资产**呈现面 + 判据面**：`ROUTES.md` 有 ★ 行且含节点 id · 判据 0.5 在场（未注册时合法跳过；**有默认层时：★ 默认层行成员/标签与 defaults.layers 一致**；**注册合法性归 `routes_contract`**、**卡文件健康归资产 `card.py check`**——不重复判） |
