...
| 版本维护层 | `version/` | **1 准则 + 4 行为**（唯一权威定义）：宿主常驻 · Skill 自主更新 · 基本知识库蒸馏 + 能力库蒸馏 · 禁止绕过主框架流程（**自定位式**：不同宿主目录/机制不同，由 agent 自行探测本宿主的文件，找不到则如实告知） |
...
- **4 行为**：① 宿主常驻 ② Skill 自主更新 ③ 基本知识库蒸馏 + 能力库蒸馏（速查卡；唯一时点 = 每日 14:00）④ 禁止绕过主框架流程；
