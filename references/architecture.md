...
- **网络兜底（内置）**：`git pull` 遇到网络错误（解析失败 / 连接超时 / 502 / 503 / 重置等）时，自动向 `manifest.deploy.accelerator_url`（默认用户自建腾讯云函数）请求可达 GitHub IP（默认 `source=all`，含 ziyou 自建探测与第三方镜像，去重测速取最快），再起一个**作用域限定在单次 git 调用的本地 CONNECT 代理**把 github 域名钉到该 IP，由**本地 git 完成拉取**——云函数只供 IP、不拉仓库。绝不写系统 hosts、无需管理员；TLS 证书校验始终开启（第三方源 IP 若被投毒会因证书不匹配失败，不泄露凭据）。`api_fallback` 可关。
