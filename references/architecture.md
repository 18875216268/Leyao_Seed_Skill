...
- **网络兜底（内置）**：`git pull` 遇到网络错误（解析失败 / 连接超时 / 502 / 503 / 重置等）时，自动向 `manifest.deploy.accelerator_url`（默认用户自建腾讯云函数）请求**全员候选 IP**（`source=all`，不指定源/关键字/域名，合并 ziyou 自建探测与第三方镜像），再**本地 TCP 443 自测、排序、选可用者**（关键域名另补本地 DNS 候选，挑本机真正可达的钉定；全不可达才回退信任云函数 IP），随后起一个**作用域限定在单次 git 调用的本地 CONNECT 代理**把 github 域名钉到选中 IP，由**本地 git 完成拉取**——云函数只供 IP、不拉仓库。绝不写系统 hosts、无需管理员；TLS 证书校验始终开启（第三方源 IP 若被投毒会因证书不匹配失败，不泄露凭据）。
